...
- (void) readjustLabels
{
	...
	if (label[0].frame.size.width > self.frame.size.width){
        ...
        {
            label[0].frame = CGRectMake(0, label[0].frame.origin.y, label[0].frame.size.width, label[0].frame.size.height);
        } else
        {
            //TOOD
            //for Right Alignment
...
#pragma mark - additional methods by Yuanfeng
- (void)setTextAlightment:(UITextAlignment)textAlight
{
    for (int i=0; i<NUM_LABELS; ++i){
		label[i].textAlignment = textAlight;
	}
    [self readjustLabels];
}
...
{
    for (int i=0; i<NUM_LABELS; ++i){
		label[i].shadowColor = shadowColor;
	}
    [self readjustLabels];
}
